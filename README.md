...
- **Qdrant** 存储和检索记忆向量，支持 Docker 和纯本地两种模式
...
宽取候选 → 相关性阈值过滤 → TTL 时间过滤 → 按 score 降序排序 → 截断返回。每条结果附带相关性评分，可通过配置调节搜索参数。
...
安装时自动向 `~/.claude/CLAUDE.md` 写入 3 条强制记忆规则，确保 Claude 在每个 Session 中主动搜索和存储记忆。
...
> 这些工具由 Claude 自动调用，通常不需要你手动操作。

## 配置

配置文件位于 `~/.agent-mem0/config.yaml`，采用 **shadow config** 机制：代码内置完整默认值，用户配置文件只需写你想覆盖的字段。
...
agent-mem0 严格区分**可再生的配置**和**不可再生的用户数据**：

```
~/.agent-mem0/              ← 配置（可再生，重新 install 即可恢复）
  ├── config.yaml
  ├── projects.json
  └── logs/

~/.local/share/agent-mem0/  ← 用户记忆数据（不可再生）
  └── qdrant_storage/
```

- `agent-mem0 uninstall`：只删除配置和产物，**不触碰记忆数据**
- `agent-mem0 uninstall --purge`：额外删除记忆数据和 Docker 容器，需要**二次确认**
